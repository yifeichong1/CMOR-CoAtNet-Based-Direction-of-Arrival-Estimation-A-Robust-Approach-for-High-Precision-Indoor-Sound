...
from torchvision.models import efficientnet_v2_s  # 导入 EfficientNetV2-S
import torch.nn as nn

class SingleEfficientNetV2(nn.Module):
    def __init__(self, num_classes):
        super(SingleEfficientNetV2, self).__init__()
        # 加载预训练的 EfficientNetV2 模型
        self.base_model = efficientnet_v2_s(weights="IMAGENET1K_V1")  # 使用 torchvision 中的预训练权重

        # 替换输入层以适应自定义通道数
        original_conv = self.base_model.features[0][0]  # 获取初始卷积层
        ...
        # 替换分类头以适应自定义类别数
        original_fc = self.base_model.classifier[1]  # 获取原始全连接层
...
# 定义单个 EfficientNetV2 网络
# class SingleEffNetV2(nn.Module):
#     def __init__(self):
#         super(SingleEffNetV2, self).__init__()
#         # 加载预训练的 EfficientNetV2-S，并移除分类层
#         base_model = efficientnet_v2_s(pretrained=True)
#         for param in base_model.parameters():
#             param.requires_grad = False
#         for param in base_model.features[6:].parameters():  # 解冻最后几个 block
...
#         return x
#
# # 定义多 EfficientNetV2 融合模型
# class MultiEffNetV2(nn.Module):
#     def __init__(self, num_classes):
#         super(MultiEffNetV2, self).__init__()
#         # 创建4个独立的 EfficientNetV2 模型，分别处理4个麦克风的数据
#         self.effnet_models = nn.ModuleList([SingleEffNetV2() for _ in range(4)])
#         # EfficientNetV2-S 的最后特征维度是 1280，每个麦克风对应一个 EffNet
#         self.fc1 = nn.Linear(1280 * 4, 512)  # 合并 4 个麦克风的特征
#         self.dropout = nn.Dropout(0.5)
#         self.fc2 = nn.Linear(512, num_classes)  # 最终分类层
#
#     def forward(self, x):
#         # x 是形状为 (batch_size, 4, channels, height, width) 的输入
#         features = [effnet(x[:, i, :, :, :]) for i, effnet in enumerate(self.effnet_models)]
...
if __name__ == "__main__":
    ...
    # 初始化模型、损失函数和优化器
    model = SingleEfficientNetV2(num_classes=num_classes)
    ...
    # 保存模型
    torch.save(model.state_dict(), 'multi_efficientnet_sound_classification.pth')

    # 保存训练过程中的结果
    save_results_to_excel(
        file_path="efficientnetv2_results.xlsx",
